...
################################################
moodle_print() {
    # Set output stream to be used, depending on verbose.
    if [[ "$VERBOSE" == "0" ]]; then
        echo $1 > /dev/null 2>&1
    else
        echo $1
...
load_properties "$PERFORMANCE_TOOL_DIRECTORY/config/defaults.properties"
load_properties "$PERFORMANCE_TOOL_DIRECTORY/config/webserver_config.properties"
